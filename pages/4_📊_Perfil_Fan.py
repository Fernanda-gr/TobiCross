...
def calcular_perfil(rows):
    ...
    for c in campos:
        vals = []
        for r in rows:
            try: val = float(r[c]) if c in r.index else 0.0
            except: val = 0.0
            vals.append(val)
...
def build_guardian_html(guardian):
    ...
    score      = float(guardian.get('score', 0))
    genres_str = ', '.join(genres_g[:2])
    title      = guardian['title']
    image      = guardian.get('image_url', '')
    return (
        ...
        f'<img src="{image}" style="width:52px;height:52px;border-radius:10px;object-fit:cover;flex-shrink:0;"/>'
        f'<div>'
        f'<p style="font-size:14px;font-weight:600;color:#CECBF6;margin-bottom:2px;">{title}</p>'
        f'<p style="font-size:11px;color:#7F77DD;">Tu espejo en otro universo</p>'
        f'<p style="font-size:10px;color:#534AB7;margin-top:3px;">⭐ {score:.1f} · {genres_str}</p>'
        f'</div></div></div>'
    )

# ── UI ────────────────────────────────────────
st.markdown('<div style="text-align:center;margin-bottom:1.5rem;"><p style="font-size:13px;color:#7F77DD;letter-spacing:2px;margin:0 0 4px;font-weight:500;">PERFIL DE FAN</p><p style="font-size:24px;font-weight:600;margin:0 0 6px;">Descubre tu arquetipo animero</p><p style="font-size:14px;color:#888;margin:0;">Ingresa 3 a 5 películas que te encanten</p></div>', unsafe_allow_html=True)
...
if generar:
    if len(peliculas) < 3:
        ...
        for p in peliculas:
            ...
            if r.empty: no_encontradas.append(p)
            else: rows.append(r.iloc[0])
