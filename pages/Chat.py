...
st.set_page_config(page_title="Chat con YOMI · TobiCross", page_icon="💬", layout="wide")
...
try:
    ...
    YOMI_AVATAR = f"data:image/png;base64,{_b64}"
except:
    YOMI_AVATAR = ""
...
def avatar_yomi():
    if YOMI_AVATAR:
        return f'<img src="{YOMI_AVATAR}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;flex-shrink:0;border:1.5px solid #534AB7;"/>'
    return '<div style="width:40px;height:40px;border-radius:50%;background:#1A1635;border:1.5px solid #534AB7;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">✦</div>'
...
def render_mensaje(msg):
    if msg['role'] == 'yomi':
        ...
        <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:20px;">
          {avatar_yomi()}
          <div style="background:#EEEDFE;border-radius:0 16px 16px 16px;padding:14px 18px;max-width:60%;">
            <p style="font-size:11px;color:#534AB7;font-weight:600;margin:0 0 6px;letter-spacing:2px;">YOMI</p>
            <p style="font-size:15px;color:#3C3489;margin:0;line-height:1.8;">{msg['content']}</p>
          </div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:32px;flex-direction:row-reverse;">
          <div style="width:40px;height:40px;border-radius:50%;background:#534AB7;
            display:flex;align-items:center;justify-content:center;font-size:16px;
            font-weight:600;color:white;flex-shrink:0;">
            {st.session_state.yomi_nombre[:1].upper() if st.session_state.yomi_nombre else '👤'}
        </div>
          <div style="background:#f5f5f5;border-radius:16px 0 16px 16px;padding:14px 18px;max-width:75%;">
            <p style="font-size:15px;color:#333;margin:0;line-height:1.8;">{msg['content']}</p>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
def render_pensando():
    ...
    <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:20px;">
      {avatar_yomi()}
      <div style="background:#EEEDFE;border-radius:0 16px 16px 16px;padding:14px 18px;">
        <p style="font-size:11px;color:#534AB7;font-weight:600;margin:0 0 6px;letter-spacing:2px;">YOMI</p>
        <p style="font-size:15px;color:#7F77DD;margin:0;font-style:italic;">✦ consultando el universo...</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
with col_chat:
    if st.session_state.yomi_procesando and st.session_state.yomi_pendiente:
        ...
        for col_n, nivel, label in opciones:
            with col_n:
    ...
    if st.session_state.yomi_mensajes and not st.session_state.yomi_procesando:
        st.markdown("")
        if st.button("⊹ ࣪ ˖ Empezar de nuevo", use_container_width=True, key="btn_reset"):
